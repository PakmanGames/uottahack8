...
// Demo-friendly component types (on-demand pricing, no extra credentials needed)
// These resources provision quickly and can be destroyed immediately with minimal cost
const DEMO_MODE_ALLOWED = new Set([
  "droplet-basic",
  "droplet-general",
  "droplet-cpu",
  "droplet-memory",
...
export function generateTerraformCode(
  ...
  const result = generateTerraformCodeWithInfo(placedComponents, order, demoMode);
  return result.code;
}

export function generateTerraformCodeWithInfo(
  placedComponents: PlacedComponent[],
  order: Order | null,
  demoMode: boolean = false
): DemoModeResult {
  // Track excluded components for demo mode
  const excludedComponents: string[] = [];
  
  // Filter components in demo mode
  let componentsToUse = placedComponents;
  if (demoMode) {
    componentsToUse = placedComponents.filter((p) => {
      const allowed = DEMO_MODE_ALLOWED.has(p.component.type);
      if (!allowed) {
        excludedComponents.push(p.component.name);
      }
      return allowed;
    });
  }
  ...
  if (componentsToUse.length === 0) {
    const emptyMessage = demoMode && placedComponents.length > 0
      ? `# Demo Mode: All ${placedComponents.length} component(s) excluded (not demo-compatible)
# Excluded: ${excludedComponents.join(", ")}
#
# Demo mode only includes: Droplets, Block Storage, Load Balancer, VPC, Firewall, Floating IP`
      : `# Drag components to the build area to generate Terraform code
...
# Order: ${order?.scenario || "No order yet"}`;
    ...
    return {
      ...
      excludedComponents,
      includedCount: 0,
      excludedCount: excludedComponents.length,
    };
  }
  ...
  componentsToUse.forEach((placed) => {
  ...
  // Add demo mode header info
  if (demoMode) {
    lines.push(`# MODE: DEMO (on-demand resources only)`);
    if (excludedComponents.length > 0) {
      lines.push(`# Excluded ${excludedComponents.length} component(s): ${excludedComponents.join(", ")}`);
    }
  }
  ...
  return {
    code: lines.join("\n"),
    excludedComponents,
    includedCount: componentsToUse.length,
    excludedCount: excludedComponents.length,
  };
}
...
function generateResourceBlock(
  ...
  switch (component.type) {
    ...
    case "droplet-cpu":
    case "droplet-memory":
      // Use minimal sizes for budget-friendly demos (see SPEC.md for details)
      const budgetDropletSizes: Record<string, string> = {
        "droplet-basic": "s-1vcpu-512mb-10gb",   // $4/mo (smallest)
        "droplet-general": "s-1vcpu-1gb",         // $6/mo
        "droplet-cpu": "c-2",                     // $42/mo (smallest CPU-optimized)
        "droplet-memory": "m-16gb",               // $84/mo (smallest memory-optimized)
      };
      const dropletSize = budgetDropletSizes[component.type] || "s-1vcpu-512mb-10gb";
    ...
    case "block-storage":
      // Use minimum 10GB volume for budget-friendly demos
      lines.push(
        `resource "digitalocean_volume" "${resourceName}" {`,
        `  name                    = "volume-${suffix}-${index}"`,
        `  region                  = "nyc3"`,
        `  size                    = 10`,  // Minimum size (10GB = $1/mo)
    ...
    case "kubernetes":
      // Use minimal K8s config for budget-friendly demos (1 node instead of 3)
      lines.push(
        ...
        "  node_pool {",
        '    name       = "worker-pool"',
        '    size       = "s-1vcpu-2gb"',  // Smallest node size
        "    node_count = 1",              // Single node for demos
        "  }",
        "}"
      );
      break;
